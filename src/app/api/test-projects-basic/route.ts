// Basic test projects API without thumbnail_url
...
export async function GET(_req: NextRequest) {
  try {
    ...
    // Query projects with manager info (no thumbnail_url)
    const rows = await db
      .select({
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('🔄 Creating basic project:', payload);
    ...
    // 1. Create project (no thumbnail_url)
