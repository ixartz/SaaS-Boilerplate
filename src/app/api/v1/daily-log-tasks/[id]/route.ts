  ...
  try {
    ...
    return await runWithOrgContext({ orgId, userId: user.id, role: membership.role }, async () => {
      const rows = await db
        ...
        .where(and(eq(dailyLogTasksSchema.id, parsedId.data.id), eq(dailyLogTasksSchema.orgId, orgId), isNull(dailyLogTasksSchema.deletedAt)));
      if (rows.length === 0) {
        return notFound('Daily log task not found');
      }
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return problemJson({ title: 'Failed to update daily log task', status: 500, detail: e?.message });
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return problemJson({ title: 'Failed to delete daily log task', status: 500, detail: e?.message });
  }
}
