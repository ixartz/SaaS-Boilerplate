...
import { Download, GripVertical, Plus, RefreshCw, X } from 'lucide-react';
    ...
    return (
      <div className="mx-auto flex max-w-4xl flex-col gap-6 py-8">
        ...
        <div className="flex items-center justify-end gap-4">
          ...
          <Button onClick={handleDownload} className="bg-orange-600 text-white hover:bg-orange-700">
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          <label htmlFor="video-files" className="block text-sm font-medium text-gray-700">
            Video Files
            <span className="ml-1 font-normal text-gray-400">(Drag to reorder)</span>
          </label>
        ...
        <Button
          ...
          className="w-full bg-orange-600 text-white shadow-md hover:bg-orange-700 disabled:bg-gray-300 disabled:shadow-none"
          size="lg"
        >
...
function VideoPreviewPlayer({ url }: { url: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="group relative aspect-video w-full overflow-hidden rounded-lg border border-gray-200 bg-black shadow-lg">
        <video
          ref={videoRef}
          src={url}
        ...
        {!isPlaying && (
          <div
            ...
            tabIndex={0}
          >
            <div className="rounded-full bg-white/90 p-4 shadow-xl transition-colors hover:bg-white">
              <div className="ml-1 size-8 border-y-[10px] border-l-[16px] border-y-transparent border-l-gray-900" />
            </div>
          </div>
        )}
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-4 opacity-0 transition-opacity group-hover:opacity-100">
          <div className="flex items-center gap-3 text-white">
            <button type="button" onClick={togglePlay} className="hover:text-orange-400">
              {isPlaying
                ? (
                    <div className="size-4 border-x-4 border-current" />
                  )
                : (
                    <div className="size-4 border-y-[6px] border-l-[10px] border-y-transparent border-l-current" />
                  )}
            </button>
            ...
            <input
              ...
              value={currentTime}
              onChange={handleSeek}
              className="h-1 flex-1 appearance-none rounded-full bg-white/30 [&::-webkit-slider-thumb]:size-3 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-orange-500"
            />
          </div>
        </div>
      </div>
...
function SortableVideoItem({ videoFile, index, onRemove, disabled }: SortableVideoItemProps) {
  ...
  return (
    ...
    >
      ...
      <span className="flex size-5 items-center justify-center rounded-full bg-orange-100 text-[10px] font-bold text-orange-600">
        {index + 1}
      </span>
      ...
      <span className="text-xs text-gray-400">
        {(videoFile.file.size / (1024 * 1024)).toFixed(1)}
        {' '}
        MB
      </span>
